...
import { TestSynthraV3SwapPay } from '../typechain/TestSynthraV3SwapPay'
...
import { TestSynthraV3ReentrantCallee } from '../typechain/TestSynthraV3ReentrantCallee'
...
describe('SynthraV3Pool', () => {
  ...
  let factory: SynthraV3Factory
  let pool: MockTimeSynthraV3Pool

  let swapTarget: TestSynthraV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeSynthraV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  // https://github.com/Synthra/uniswap-v3-core/issues/214
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestSynthraV3ReentrantCallee')
      ).deploy()) as TestSynthraV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestSynthraV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestSynthraV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestSynthraV3SwapPay
